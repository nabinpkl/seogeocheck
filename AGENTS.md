...
- **MCP Server Slice (`backend/`):** `/mcp` is implemented as a Java MCP Streamable HTTP server on the existing Spring Security resource-server layer. V1 is tools-only and exposes authenticated, owner-scoped project/audit operations. MCP tool handlers must resolve ownership from the OAuth bearer token only, never from user-supplied identifiers. Access tokens for MCP must include the canonical MCP resource audience (`issuer-without-trailing-slash + "/mcp"`), and `/mcp` must reject tokens missing that audience even when scope and other claims are valid.
