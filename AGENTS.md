...
- **OAuth MCP Slice (`backend/`):** The backend also hosts a first-party OAuth 2.1 authorization server for delegated MCP access. `/mcp` is a bearer-token-only protected resource backed by the same backend, while `/oauth2/authorize`, `/oauth2/token`, `/oauth2/revoke`, JWKs, authorization server metadata, and OAuth protected-resource metadata are published from the Java tier. V1 supports manual client registration, `authorization_code` + PKCE with `S256`, rotating refresh tokens for public clients, backend-rendered consent, and verified non-anonymous user approval only. Browser sign-in still reuses the frontend sign-in page, but browser sessions must never authorize `/mcp` directly without an exchanged bearer token.
