...
- **OAuth MCP Slice (`backend/`):** The backend also hosts a first-party OAuth 2.1 authorization server for delegated MCP access. `/mcp` is a bearer-token-only protected resource backed by the same backend, while `/oauth2/authorize`, `/oauth2/token`, `/oauth2/revoke`, `/oauth2/register`, JWKs, authorization server metadata, and OAuth protected-resource metadata are published from the Java tier. OAuth clients are database-backed through Spring Authorization Server JDBC. V1 uses constrained open Dynamic Client Registration at `/oauth2/register` so tools such as Claude Code and Codex can register public PKCE clients with their real redirect URIs before authorization without any SEOGEO-specific bootstrap endpoint or pre-shared registrar client. Registered clients are limited to `authorization_code` + PKCE with `S256`, rotating refresh tokens, backend-rendered consent, verified non-anonymous user approval, the MCP scope only, and strict redirect URI validation. Browser sign-in still reuses the frontend sign-in page, but browser sessions must never authorize `/mcp` directly without an exchanged bearer token.
- **MCP Server Slice (`backend/`):** `/mcp` is implemented as a Java MCP Streamable HTTP server on the existing Spring Security resource-server layer. V1 exposes authenticated, owner-scoped project/audit tools plus a read-only discovery resource at `seogeo://guide/overview`. MCP-native metadata must explain what SEOGEO offers, the normal audit workflow, and the real input/output schemas so generic agents can understand the server without repo-only context. MCP tool handlers must resolve ownership from the OAuth bearer token only, never from user-supplied identifiers. Access tokens for MCP must include the canonical MCP resource audience (`issuer-without-trailing-slash + "/mcp"`), and `/mcp` must reject tokens missing that audience even when scope and other claims are valid.
...
- **Default Project Slice (`backend/` + `frontend/`):** Every owned workspace has one visible, non-deletable default project. Every owned audit must belong to exactly one project, with backend-owned placement into the selected project or the default project when none is specified. Project history is always shaped as `project -> unique normalized URLs -> audit history`, so repeated audits for the same normalized URL collapse into one tracked URL bucket per project while the same normalized URL may still exist in other projects with separate history.
...
- Audit experience rule: homepage audits, dashboard audit starts, and project audit routes must share one reusable audit-experience controller/view composition. Do not fork the live-progress, SSE, report-polling, or live-to-final transition logic into separate per-screen implementations.
