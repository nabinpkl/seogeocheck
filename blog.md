...
Most side projects definitely do not need an API tier, a Java worker tier, and a Node worker tier just to run Lighthouse and parse a page.
...
4. A Java activity calls a Node Lighthouse sidecar over HTTP.
5. The sidecar launches Chrome, runs Lighthouse, normalizes the result, and returns JSON.
...
Java wants Lighthouse results.
Node is good at Lighthouse.
HTTP is simple.
...
If the Java Temporal worker calls a Lighthouse sidecar over HTTP, then scaling Java workers does not automatically give you a clean model for scaling Lighthouse capacity.

You can add more Java worker instances, but those workers still funnel Lighthouse execution through an external HTTP service. That means the browser-heavy tier is still being treated like a remote dependency, not like a first-class execution pool managed by Temporal.

In other words:

- Java scales as workers
- Lighthouse scales as a service endpoint
...
In a more Temporal-native design, the retryable unit is not "send an HTTP request to something that runs Lighthouse."

It is "run the Lighthouse activity on the Node worker queue."
...
Chrome is not a lightweight dependency. If you run too many Lighthouse jobs concurrently on the same machine, performance collapses quickly. CPU spikes, memory climbs, and the machine starts thrashing.

That makes Lighthouse capacity different from Java orchestration capacity.
...
If Lighthouse becomes the bottleneck, I can scale Node workers.
...
That makes the Lighthouse tier less visible as a first-class execution boundary.
...
If too many Lighthouse jobs run concurrently, the Node workers will struggle long before the Java workers do. That is exactly why browser execution deserves its own worker tier.
...
- Node owns Lighthouse and browser-executed work
...
- Node workers handle Lighthouse and browser execution
